...
#             ns.noise       = 0  # add noise to ISI? (0-1)
#             # TODO change number of epsp
#             ns.number      = 20 # numer of sequential psps
#
#             # create NetCon object
#             nc             = h.NetCon(ns,syn)
#             nc.delay       = 0
...
#         h.fadvance()
#
#     fig,ax = plt.subplots(count,2, figsize=(6,10), sharex='all')
#
#     for cv in range(count):
#         ax[cv][0].plot(tm.to_python(), vm.to_python(), label='soma')
#         ax[cv][0].plot(tm.to_python(), vml.to_python(), label=c.name())
#         ax[cv][0].set_title('Voltage in the soma and a stimulated dendrite')
#         ax[cv][0].set_ylabel('Voltage (mV)')
#         ax[cv][0].legend()
#
#         ax[cv][1].plot(tm, ampa_current, label='ampa')
#         ax[cv][1].plot(tm, nmda_current, label='nmda')
#
#         ax[cv][1].set_title('synaptic current')
#         ax[cv][1].set_ylabel('Current (nA)')
#         ax[cv][1].set_xlabel('Time (ms)')
#         ax[cv][1].legend()
#
#     plt.show()
#
